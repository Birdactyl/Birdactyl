...
func GetUserResourceUsage(userID uuid.UUID) ResourceUsage {
	cacheKey := "resource_usage_" + userID.String()
	if cached, found := Cache.Get(cacheKey); found {
		return cached.(ResourceUsage)
	}
	...
	Cache.Set(cacheKey, usage, 30*time.Second)
	return usage
}
...
func GetServersByUser(userID uuid.UUID) ([]models.Server, error) {
	cacheKey := "servers_" + userID.String()
	var servers []models.Server

	if cached, found := Cache.Get(cacheKey); found {
		servers = cached.([]models.Server)
	} else {
		...
		Cache.Set(cacheKey, servers, 30*time.Second)
	}
	...
	for i := range result {
		if stats := GetServerStats(result[i].ID); stats != nil {
			result[i].Status = models.ServerStatus(stats.State)
		}
	}
