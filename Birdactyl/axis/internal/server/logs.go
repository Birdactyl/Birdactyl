...
var progressRegex = regexp.MustCompile(`^\s*\d+%\s*[\xe2\x96\x80-\xe2\x96\x88\x89\xe2\x96\x91-\xff|#=\-\[\]\\/>]+\s*$`)
var apkProgressRegex = regexp.MustCompile(`^\s*\d+\s+\d+%\s+[\xe2\x96\x80-\xff]+`)
...
func isProgressLine(line string) bool {
	...
	if progressRegex.MatchString(trimmed) {
		return true
	}
	if apkProgressRegex.MatchString(trimmed) {
		return true
	}
	allBlock := true
