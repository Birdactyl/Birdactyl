...
func main() {
	...
	engineName := strings.Title(cfg.Node.ContainerEngine)
	logger.Info("Initializing %s...", engineName)
	if err := docker.Init(cfg.Node.ContainerEngine, cfg.Node.DockerSocket); err != nil {
		logger.Fatal("%s initialization failed: %v", engineName, err)
	}
	logger.Success("%s ready", engineName)

	if cfg.Panel.Token == "" {
		logger.Fatal("Panel token not configured. Create a node in the panel and add the token to config.yaml")
	}

	if resolved := system.ResolveWSLPanelURL(cfg.Panel.URL); resolved != cfg.Panel.URL {
		logger.Info("WSL detected, rewriting panel URL: %s -> %s", cfg.Panel.URL, resolved)
		cfg.Panel.URL = resolved
	}
