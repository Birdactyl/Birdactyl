...
import { notify, SlidePanel, Input, Button, Icons, Checkbox, ContextMenu } from '../';
...
export default function ScheduleModal({ serverId, schedule, open, onClose, onSaved }: Props) {
    ...
    return (
        <SlidePanel
            ...
            description="Configure when and what actions to run."
            footer={
                <div className="flex justify-end gap-3">
                    <Button variant="ghost" onClick={onClose} disabled={loading}>Cancel</Button>
                    <Button onClick={() => formRef.current?.requestSubmit()} loading={loading} disabled={!name || loading}>{schedule ? 'Save' : 'Create'}</Button>
                </div>
            }
        >
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <ContextMenu
                        ...
                        items={[
                            ...CRON_PRESETS.map(p => ({ label: p.label, onClick: () => { setCron(p.value); setUseCustomCron(false); } })),
                            { label: 'Custom...', onClick: () => setUseCustomCron(true) },
                        ]}
                    />
                    {useCustomCron && (
                        <div className="mt-2">
                                ...
                                <div key={idx} className="p-3 rounded-lg bg-neutral-800/50 border border-neutral-700 space-y-3">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <ContextMenu
                                                ...
                                                items={TASK_ACTIONS.map(a => ({ label: a.label, onClick: () => updateTask(idx, 'action', a.value) }))}
                                            />
                                        </div>
                                    ...
                                    {task.action === 'power' && (
                                        <ContextMenu
                                            ...
                                            items={POWER_OPTIONS.map(p => ({ label: p.label, onClick: () => updateTask(idx, 'payload', p.value) }))}
                                        />
                                    )}
