...
export default function AdminCreateServerModal({ nodes, packages, open, onClose, onCreated }: Props) {
  ...
  return (
    <SlidePanel
      ...
      onClose={onClose}
      title="Create Server"
      description="Create a new server. Leave User ID blank to create for yourself."
      footer={
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={onClose} disabled={loading}>Cancel</Button>
          <Button onClick={() => formRef.current?.requestSubmit()} loading={loading} disabled={!name || !nodeId || !packageId}>Create</Button>
        </div>
      }
    >
