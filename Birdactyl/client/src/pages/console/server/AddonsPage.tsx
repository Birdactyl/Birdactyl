...
import { Button, Icons, Modal, Input, PermissionDenied, Table, ContextMenu } from '../../../components';
        ...
        <div className="flex flex-wrap items-center gap-2">
          <ContextMenu
            ...
            items={sources.map(src => ({
              label: src.name,
              icon: src.icon ? <img src={src.icon} alt="" className="w-4 h-4 rounded" /> : undefined,
              onClick: () => { setActiveSource(src); setAddons([]); setSearch(''); setExpanded(new Set()); setPage(1); setTab('browse'); },
            }))}
          />
          {activeSource?.type !== 'modpack' && (
            <ContextMenu
              ...
              items={[
                { label: 'Browse', onClick: () => setTab('browse') },
                { label: 'Installed', onClick: () => setTab('installed') },
              ]}
            />
          )}
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-neutral-400">
            <div className="flex items-center gap-2">
              ...
              <ContextMenu
                ...
                items={[10, 20, 50, 100].map(n => ({ label: String(n), onClick: () => handlePerPageChange(n) }))}
              />
            </div>
      ...
      <div className="bg-neutral-900/40 rounded-lg p-1">
        {tab === 'browse' ? (
          <Table
            ...
            contextMenu={(addon: Addon) => [
              { label: 'Install', onClick: () => openVersions(addon) },
            ]}
          />
        ) : (
          <Table
            ...
            contextMenu={can('file.delete') ? (file: InstalledAddon) => [
              { label: 'Delete', onClick: () => setDeleteFile(file), variant: 'danger' as const },
            ] : undefined}
          />
        )}
      </div>
