...
import { PermissionDenied, ContextMenu, Table } from '../../../components';
...
export default function NetworkPage() {
  ...
  const getAllocationActions = (alloc: Allocation) => [
    ...(can('allocation.set_primary') && !alloc.primary ? [{ label: 'Set primary', onClick: () => handleSetPrimary(alloc.port) }] : []),
    ...(can('allocation.delete') && !alloc.primary ? [{ label: 'Delete', onClick: () => handleDelete(alloc.port), variant: 'danger' as const }] : []),
    ...(alloc.primary ? [{ label: 'Primary allocation', disabled: true }] : []),
  ];
      ...
      <div className="rounded-xl bg-neutral-800/30">
        ...
        <div className="bg-neutral-900/40 rounded-lg p-1">
          <Table
            columns={[
              ...
              { key: 'port', header: 'Port', render: (alloc: Allocation) => <span className="text-sm text-neutral-300 tabular-nums">{alloc.port}</span> },
              {
                key: 'actions', header: '', align: 'right' as const, render: (alloc: Allocation) => (
                  <ContextMenu
                    align="end"
                    trigger={<Button variant="ghost" disabled={loading}><Icons.ellipsis className="w-5 h-5" /></Button>}
                    items={getAllocationActions(alloc)}
                  />
                )
              },
            ]}
            ...
            contextMenu={getAllocationActions}
          />
        </div>
      </div>
    </div>
