...
import { notify, Button, Modal, Icons, Table, ContextMenu } from '../../../components';
...
export default function DatabaseHostPage() {
  ...
  const getDbActions = (db: HostDatabase) => [
    { label: 'Delete', onClick: () => setDeleteModal(db), variant: 'danger' as const },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-neutral-800/30">
        <div className="bg-neutral-900/40 rounded-lg p-1">
          <Table columns={columns} data={databases} keyField="id" emptyText="No databases on this host" contextMenu={getDbActions} />
        </div>
      </div>
