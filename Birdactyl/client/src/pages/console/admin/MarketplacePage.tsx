...
import { Icons, Input, Table, ContextMenu, Button, Modal } from '../../../components';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <ContextMenu
          ...
          items={[
            { label: 'Browse', onClick: () => { setTab('browse'); setSearch(''); } },
            { label: 'Installed', onClick: () => { setTab('installed'); setSearch(''); } },
          ]}
        />
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-400">
          <div className="flex items-center gap-2">
            ...
            <ContextMenu
              ...
              items={[10, 20, 50, 100].map(n => ({ label: String(n), onClick: () => { setPerPage(n); setPage(1); } }))}
            />
          </div>
      ...
      <div className="bg-neutral-900/40 rounded-lg p-1">
        {tab === 'browse' ? (
          <Table
            ...
            contextMenu={(repo: GitHubRepo) => [
              { label: 'Install', onClick: () => openInstallModal(repo) },
              { label: 'View on GitHub', onClick: () => window.open(repo.html_url, '_blank') },
            ]}
          />
        ) : (
          <Table
            ...
            contextMenu={(file: PluginFile) => [
              { label: 'Delete', onClick: () => setDeleteModal({ file, loading: false }), variant: 'danger' as const },
            ]}
          />
        )}
      </div>
