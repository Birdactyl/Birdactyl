...
import { notify, Button, Input, Modal, CreatePackageModal, Icons, Pagination, ContextMenuZone } from '../../../components';
          ...
          <div className="rounded-xl bg-neutral-800/30">
            ...
            <div className="bg-neutral-900/40 rounded-lg p-1">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {paginated.map(pkg => (
                  <ContextMenuZone key={pkg.id} items={[
                    { label: 'Edit', onClick: () => setEditPackage(pkg) },
                    { label: 'Delete', onClick: () => setConfirmDelete({ pkg, loading: false }), variant: 'danger' as const },
                  ]}>
                    <PackageCard pkg={pkg} onDelete={() => setConfirmDelete({ pkg, loading: false })} onEdit={() => setEditPackage(pkg)} />
                  </ContextMenuZone>
                ))}
              </div>
            </div>
