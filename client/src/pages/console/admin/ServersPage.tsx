...
import { notify, Button, Input, Checkbox, Icons, ContextMenu, BulkActionBar, Table, Pagination } from '../../../components';
...
export default function ServersPage() {
  ...
  const getServerActions = (server: Server) => [
    { label: 'View', onClick: () => viewServer(server) },
    { label: 'Edit', onClick: () => setEditServer(server) },
    ...(!server.is_suspended && server.status !== 'running' ? [{ label: 'Start', onClick: () => handlePower(server, 'start') }] : []),
    ...(!server.is_suspended && server.status === 'running' ? [{ label: 'Stop', onClick: () => handlePower(server, 'stop') }] : []),
    ...(!server.is_suspended && server.status === 'running' ? [{ label: 'Kill', onClick: () => handlePower(server, 'kill') }] : []),
    ...(server.is_suspended
      ? [{ label: 'Unsuspend', onClick: () => setConfirmAction({ type: 'unsuspend', ids: [server.id] }) }]
      : [{ label: 'Suspend', onClick: () => setConfirmAction({ type: 'suspend', ids: [server.id] }) }]
    ),
    { label: 'Delete', onClick: () => setConfirmAction({ type: 'delete', ids: [server.id] }), variant: 'danger' as const },
  ];
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1">
            ...
            <div className="flex items-center gap-3">
              <ContextMenu
                ...
                items={[
                  { label: 'All Servers', onClick: () => table.handleFilterChange('all') },
                  { label: 'Running', onClick: () => table.handleFilterChange('running') },
                  { label: 'Stopped', onClick: () => table.handleFilterChange('stopped') },
                  { label: 'Suspended', onClick: () => table.handleFilterChange('suspended') },
                ]}
              />
              <ContextMenu
                align="start"
                ...
                items={[
                  { label: 'On', onClick: () => !serverCreationEnabled && toggleServerCreation() },
                  { label: 'Off', onClick: () => serverCreationEnabled && toggleServerCreation() },
                ]}
              />
            </div>
          </div>
        ...
        <div className="rounded-xl bg-neutral-800/30">
          ...
          <div className="bg-neutral-900/40 rounded-lg p-1">
            <Table columns={columns} data={table.items} keyField="id" loading={table.loading} emptyText="No servers found" rowClassName={server => table.selected.has(server.id) ? 'bg-neutral-800/20' : ''} contextMenu={getServerActions} />
          </div>
        </div>
      </div>
