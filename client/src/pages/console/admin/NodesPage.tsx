...
import { notify, Button, Input, Modal, SlidePanel, Icons, ContextMenu, Table, Pagination } from '../../../components';
...
export default function NodesPage() {
  ...
  const getNodeActions = (node: Node) => [
    { label: 'Edit', onClick: () => setEditModal({ open: true, loading: false, node, name: node.name, icon: node.icon || '' }) },
    { label: 'Reset Token', onClick: () => handleResetToken(node) },
    { label: 'Delete', onClick: () => setDeleteModal({ node, loading: false }), variant: 'danger' as const },
  ];
  ...
  return (
    <>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1">
            ...
            <div className="flex items-center gap-3">
              <ContextMenu
                ...
                items={[
                  { label: 'All Nodes', onClick: () => setUi(s => ({ ...s, filter: 'all', page: 1 })) },
                  { label: 'Online', onClick: () => setUi(s => ({ ...s, filter: 'online', page: 1 })) },
                  { label: 'Offline', onClick: () => setUi(s => ({ ...s, filter: 'offline', page: 1 })) },
                ]}
              />
      ...
      <SlidePanel
        open={pairModal.open}
        onClose={() => !pairModal.loading && setPairModal(s => ({ ...s, open: false, stage: 'form', code: '', error: '' }))}
        title="Pair Node"
        description={pairModal.stage === 'form' ? 'Connect to a node running in pairing mode.' : 'Waiting for confirmation on the node...'}
        footer={
          pairModal.stage === 'form' ? (
            <div className="flex justify-end gap-3">
              <Button variant="ghost" onClick={() => setPairModal(s => ({ ...s, open: false }))}>Cancel</Button>
              <Button onClick={() => { const form = document.getElementById('pair-form') as HTMLFormElement; form?.requestSubmit(); }} loading={pairModal.loading}>Start Pairing</Button>
            </div>
          ) : pairModal.error ? (
            <div className="flex justify-end gap-3">
      ...
      >
        {pairModal.stage === 'form' ? (
          <form id="pair-form" onSubmit={handleStartPairing} className="space-y-4">
          ...
          <div className="space-y-6">
            <div className="text-center py-6">
              <div className="text-xs text-neutral-400 mb-3">Verify this code matches on the node terminal:</div>
            ...
            {pairModal.error ? (
              ...
              <div className="flex items-center justify-center gap-2 text-neutral-400 py-4">
