...
import { notify, Button, Input, Modal, Icons, Table, ContextMenu } from '../../../components';
...
export default function DatabaseHostsPage() {
  ...
  const getHostActions = (h: AdminDatabaseHost) => [
    { label: 'View Databases', onClick: () => navigate(`/console/admin/database-hosts/${h.id}`) },
    { label: 'Delete', onClick: () => setDeleteModal(h), disabled: h.databases > 0, variant: 'danger' as const },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-neutral-800/30">
        ...
        <div className="bg-neutral-900/40 rounded-lg p-1">
          <Table columns={columns} data={hosts} keyField="id" emptyText="No database hosts configured" contextMenu={getHostActions} />
        </div>
      </div>
