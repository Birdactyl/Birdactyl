...
import { Button, Icons, Table, PermissionDenied, ContextMenu } from '../../../components';
...
export default function SchedulesPage() {
    ...
    const getScheduleActions = (s: Schedule) => [
        ...(can('schedule.run') ? [{ label: 'Run Now', onClick: () => handleRunNow(s) }] : []),
        ...(can('schedule.update') ? [{ label: 'Edit', onClick: () => setEditModal({ open: true, schedule: s }) }] : []),
        ...(can('schedule.delete') ? [{ label: 'Delete', onClick: () => setDeleteModal(s), variant: 'danger' as const }] : []),
    ];
            ...
            <div className="rounded-xl bg-neutral-800/30">
                ...
                <div className="bg-neutral-900/40 rounded-lg p-1">
                    <Table
                        ...
                        contextMenu={getScheduleActions}
                    />
                </div>
            </div>
