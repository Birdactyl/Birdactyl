...
import { notify, SlidePanel, Input, Button, Icons, ContextMenu } from '../';
...
export default function EditServerModal({ server, nodes, open, onClose, onSaved, onTransferStarted }: Props) {
  ...
  const cachedServer = useRef<Server | null>(null);

  useEffect(() => {
    if (server && open) {
      cachedServer.current = server;
  ...
  const activeServer = server ?? cachedServer.current;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (submittingRef.current || !activeServer) return;
    submittingRef.current = true;
    setLoading(true);

    if (tab === 'transfer') {
      if (!targetNodeId || targetNodeId === activeServer.node_id) {
        notify('Info', 'Select a different node to transfer', 'info');
        setLoading(false);
        submittingRef.current = false;
        return;
      }
      const res = await adminTransferServer(activeServer.id, targetNodeId);
      if (res.success && res.data) {
        const initialStatus: TransferStatus = {
          id: res.data.transfer_id,
          server_id: activeServer.id,
          server_name: activeServer.name,
          from_node_id: activeServer.node_id,
          from_node_name: activeServer.node?.name || '',
    ...
    if (name !== activeServer.name) data.name = name;
    if (ownerId !== activeServer.user_id) data.user_id = ownerId;
    const mem = parseInt(memory);
    const cpuVal = parseInt(cpu);
    const diskVal = parseInt(disk);
    if (mem && mem !== activeServer.memory) data.memory = mem;
    if (cpuVal && cpuVal !== activeServer.cpu) data.cpu = cpuVal;
    if (diskVal && diskVal !== activeServer.disk) data.disk = diskVal;
    ...
    const res = await adminUpdateServerResources(activeServer.id, data);
    if (res.success) {
  ...
  const availableNodes = activeServer ? nodes.filter(n => n.id !== activeServer.node_id && n.is_online) : [];
  const offlineNodes = activeServer ? nodes.filter(n => n.id !== activeServer.node_id && !n.is_online) : [];
    ...
    <SlidePanel
      ...
      footer={
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={() => formRef.current?.requestSubmit()} loading={loading}>{tab === 'transfer' ? 'Transfer Server' : 'Save Changes'}</Button>
        </div>
      }
    >
      {activeServer && (
              ...
              <>
                ...
                <div className="pt-2 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-neutral-300">{activeServer.user?.username || '\u2014'}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-neutral-400">Node</span>
                    <span className="text-neutral-300">{activeServer.node?.name || '\u2014'}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-neutral-400">Server ID</span>
                    <span className="text-neutral-500 font-mono text-[10px]">{activeServer.id}</span>
                  </div>
                </div>
              </>
            )}
            ...
            {tab === 'transfer' && (
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="px-3 py-2 rounded-lg bg-neutral-800/50 text-sm text-neutral-300">{activeServer.node?.name || '\u2014'}</div>
                </div>
                <div className="flex flex-col gap-1.5">
