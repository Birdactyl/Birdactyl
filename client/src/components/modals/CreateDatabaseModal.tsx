...
import { SlidePanel, Input, Button, Icons, ContextMenu } from '../';
...
export default function CreateDatabaseModal({ serverId, open, onClose, onCreated }: Props) {
  ...
  return (
    <SlidePanel
      ...
      description="Create a new MySQL database for this server."
      footer={
          ...
          {hosts.length === 0 ? <p className="text-neutral-500 text-sm">No database hosts available.</p> : (
            <ContextMenu
              ...
              items={hosts.map(h => {
                ...
                return {
                  label: `${h.name} (${h.host}:${h.port}) - ${h.max_databases > 0 ? `${h.used}/${h.max_databases}` : h.used} used`,
                  onClick: () => !full && setSelectedHost(h),
                  disabled: full,
                };
              })}
            />
