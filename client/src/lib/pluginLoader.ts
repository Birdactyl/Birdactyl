...
/** @deprecated Use PluginUINavItem instead. */
export type PluginUISidebarItem = PluginUINavItem;
...
export interface PluginUIManifest {
  ...
  pages: PluginUIPage[];
  tabs: PluginUITab[];
  /** Navigation items. Server may send as sidebarItems (proto wire name) or navItems. */
  navItems: PluginUINavItem[];
  /** Wire compat: server sends sidebar_items which deserializes as sidebarItems */
  sidebarItems?: PluginUINavItem[];
}
...
export async function loadPlugin(manifest: PluginUIManifest): Promise<LoadedPlugin | null> {
  ...
  const loadPromise = (async () => {
    ...
    const user = getUser();
    const items = manifest.navItems || manifest.sidebarItems || [];
    ...
    if (module) {
      for (const page of manifest.pages) {
        const comp = module[page.component];
        if (!comp) {
          console.warn(`[plugins] Component "${page.component}" not found in bundle for ${manifest.id}`);
          continue;
        }
        const canAccess = checkGuard(page.guard, user, evaluateGuard);
        if (!canAccess) continue;
        const routePath = page.path.replace(/^\/console/, '');
        const pluginId = manifest.id;
        const componentName = page.component;
        const lazyComp = lazy(() => import('../components/plugins/PluginRenderer').then(mod => ({
          default: () => createElement(mod.default, { pluginId, component: componentName }),
        })));
        registry.registerPage({
          path: routePath,
          component: lazyComp,
          guard: page.guard as 'admin' | null,
        });
      }
    }
...
export function getPluginNavItems(): PluginUINavItem[] {
  const items: PluginUINavItem[] = [];
  for (const plugin of loadedPlugins.values()) {
    items.push(...(plugin.manifest.navItems || plugin.manifest.sidebarItems || []));
  }
  return items.sort((a, b) => (a.order || 0) - (b.order || 0));
}

/** @deprecated Use getPluginNavItems instead. */
export const getPluginSidebarItems = getPluginNavItems;
