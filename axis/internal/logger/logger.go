...
func Debug(format string, args ...interface{}) {
	log(Gray, "DEBUG", fmt.Sprintf(format, args...))
}
...
func Server(serverID, msg string) {
	ts := time.Now().Format("15:04:05")
	colored := fmt.Sprintf("%s%s%s %s[%s]%s %s", Gray, ts, Reset, Magenta, serverID, Reset, msg)
	plain := fmt.Sprintf("%s [%s] %s", ts, serverID, msg)
...
func Transfer(format string, args ...interface{}) {
	log(Blue, "TRANSFER", fmt.Sprintf(format, args...))
}
