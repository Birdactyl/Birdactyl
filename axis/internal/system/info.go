...
func ResolveWSLPanelURL(panelURL string) string {
	if !IsWSL() {
		return panelURL
	}
	...
	resolved := replacer.Replace(panelURL)

	if strings.HasSuffix(panelURL, "://localhost") {
		resolved = strings.TrimSuffix(panelURL, "://localhost") + "://" + hostIP
	}
	if strings.HasSuffix(panelURL, "://127.0.0.1") {
		resolved = strings.TrimSuffix(panelURL, "://127.0.0.1") + "://" + hostIP
	}
