...
While GORM handles most migrations automatically, changing column types (e.g., from `varchar` to `text`) may not be executed automatically in PostgreSQL and some other databases. If you encounter errors relating to string length (like "value too long for type character varying"), you may need to manually update the column type via an SQL raw query:

```sql
ALTER TABLE activity_logs ALTER COLUMN description TYPE text;
```
