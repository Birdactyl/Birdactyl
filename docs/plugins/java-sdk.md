...
Add custom pages, tabs, and topbar items to the panel. The UI is built using React components bundled with your plugin.
...
public MyPlugin() {
    ...
    setUI(ui()
        ...
        .tab("my-tab", "ServerTab", UIBuilder.TAB_TARGET_SERVER, "My Tab")
            ...
            .order(100)
            .done()
        .topbarItem("my-plugin", "My Plugin", "/plugins/my-plugin", UIBuilder.NAV_SECTION_NAV)
            .icon("puzzle")
            .order(50)
            .done()
        .build());
...
Navigation sections:
- `UIBuilder.NAV_SECTION_NAV` - Main navigation
- `UIBuilder.NAV_SECTION_PLATFORM` - Platform section
- `UIBuilder.NAV_SECTION_ADMIN` - Admin section
