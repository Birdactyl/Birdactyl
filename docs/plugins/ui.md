...
Plugins can add custom user interfaces to the panel, including pages, tabs, and topbar items. The UI is built with React and TypeScript using the `@birdactyl/plugin-ui` SDK.
...
func main() {
    ...
    plugin.UI().
        ...
        NavItem("my-plugin", "My Plugin", "/plugins/my-plugin", birdactyl.NavSectionPlatform).
    ...
    public static void main(String[] args) throws Exception {
        ...
        UIBuilder ui = plugin.ui()
            ...
            .tab("settings-tab", "SettingsTab", UIBuilder.TAB_TARGET_USER_SETTINGS, "My Plugin")
                .icon("puzzle").order(10).done()
            .topbarItem("my-plugin", "My Plugin", "/plugins/my-plugin", UIBuilder.NAV_SECTION_PLATFORM)
                .icon("puzzle").order(50)
                .child("Dashboard", "/plugins/my-plugin")
                .child("Settings", "/plugins/my-plugin/settings")
                .done();
...
plugin.UI().
    NavItem("my-plugin", "My Plugin", "/plugins/my-plugin", birdactyl.NavSectionPlatform).
        ...
        Child("Settings", "/plugins/my-plugin/settings").
        Done().
    NavItem("my-plugin-admin", "Admin", "/plugins/my-plugin/admin", birdactyl.NavSectionAdmin).
...
```java
ui.topbarItem("my-plugin", "My Plugin", "/plugins/my-plugin", UIBuilder.NAV_SECTION_PLATFORM)
    .icon("puzzle").order(50)
    .child("Dashboard", "/plugins/my-plugin")
    .child("Settings", "/plugins/my-plugin/settings")
    .done()
  .topbarItem("my-plugin-admin", "Admin", "/plugins/my-plugin/admin", UIBuilder.NAV_SECTION_ADMIN)
    .icon("shield").adminOnly().done();
```

| Section | Go Constant | Java Constant | Description |
|---------|-------------|---------------|-------------|
| `nav` | `NavSectionNav` | `NAV_SECTION_NAV` | Main navigation section |
| `platform` | `NavSectionPlatform` | `NAV_SECTION_PLATFORM` | Platform section (below servers) |
| `admin` | `NavSectionAdmin` | `NAV_SECTION_ADMIN` | Admin section (admin users only) |

| Method | Description |
|--------|-------------|
| `topbarItem(id, label, href, section)` | Create a topbar item |
